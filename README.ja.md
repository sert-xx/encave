...
最初の対象は **Codex CLI** ですが、中核となる考え方 — 隔離されたエージェントホーム +
起動時の認証情報注入 + git clone とタグによる配布 — は対象非依存であり、アダプタ
インターフェースの背後に置かれているため、他のエージェント CLI も後から追加できます。
...
インストールし、起動時に認証情報を注入して実行します。これにより受け取る側の個人ホームは
一切変更されません。
...
- **隔離**: インストールされた各エージェントは `<root>/<owner>/<repo>` 配下に置かれ、
  それ専用のホームディレクトリ（Codex なら `CODEX_HOME`）で起動されます。あなたの
  `~/.codex` には触れません。
- **秘密情報は git に入れない**: 秘密でない設定（プロバイダの `base_url`、認証用環境変数の
  *名前*、`wire_api` など）はコミットされます。実際の認証情報は **OS キーリング** に保存され、
  起動された子プロセスの環境にのみ注入されます。リポジトリ・ログ・標準出力には決して
  書き出されません。

保存された認証情報を **表示するコマンドは意図的に用意していません**。それは恒常的な
認証情報ダンプの窓口になってしまうからです。認証情報がキーリングから出るのは、起動された
1 つの子プロセスの環境としてのみで、そのプロセスが生きている間だけです。
...
- 実行したい **対象エージェント CLI** — 現状は **Codex CLI**。
- 動作する **OS キーリング**: macOS Keychain と Windows Credential Manager はそのまま
  使えます。**Linux** では `gnome-keyring` や KeePassXC の Secret Service のような、
  動作中の **Secret Service** が必要です。
...
- `internal/adapter` — 対象 CLI の抽象化。`codex.go` が最初のアダプタ（ホーム環境変数
  `CODEX_HOME`、`env_key` / `env_http_headers` を読んで注入すべき環境変数を判断し、
  `codex -c key=value` のオーバーライドを構築）。
...
v1: Codex CLI、静的/長命の認証情報（例: 30 日の PAT）を起動時に注入する単一カスタム
プロバイダ。次の予定: 汎用 Codex 認証（ChatGPT ログイン / API キー）、Claude Code
アダプタ、エージェントごとの複数認証情報。
