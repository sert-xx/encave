...
どちらのチェックも **git のリリースタグ**を直接読みます（encave 自身はそのリポジトリから、
エージェントは `origin` から）。そのため新しく push されたタグもすぐ検出されます — モジュール
プロキシの `@latest` のように遅延しません。ネットワーク確認は対象ごとに最大 1 時間に 1 回
程度ですが、いったん更新を把握したら、インストールするかその版を拒否するまで毎回プロンプト
を出します（より新しい版が出ればまた出します）。チェックは端末に接続されている場合のみ実行、
自己チェックにはリリース版（`make build` の開発用バイナリではない）が必要で、処理をブロック
せず、ネットワークエラーは黙ってスキップします。すべて無効にするには
...
- `internal/gitutil` — `git` CLI の薄いラッパー（更新チェックの `ls-remote`/`fetch`
  によるタグ取得を含む）。
